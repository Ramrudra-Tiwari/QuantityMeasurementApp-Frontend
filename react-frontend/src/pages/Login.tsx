...
async function doSignup() {
  ...
  if (!signupName) {
    ...
    valid = false;
  } else setSignupNameErr(false);

  if (!isValidEmail(signupEmail)) {
    setSignupEmailErr(true);
    valid = false;
  } else setSignupEmailErr(false);

  if (!PASSWORD_REGEX.test(signupPw)) {
    setSignupPwErr(true);
    valid = false;
  } else setSignupPwErr(false);

  if (!/^\d{10}$/.test(signupMobile)) {
    setSignupMobileErr(true);
    valid = false;
  } else setSignupMobileErr(false);
        ...
        <div className="form-section">
          ...
          {activeTab === 'login' && (
            <div className="form-panel active">
              ...
              <p className="switch-link">Don't have an account? <a href="#!" onClick={(e) => { e.preventDefault(); setActiveTab('signup'); }}>Sign Up</a></p>
          ...
          {activeTab === 'signup' && (
            <div className="form-panel active">
              ...
              <p className="switch-link">Already have an account? <a href="#!" onClick={(e) => { e.preventDefault(); setActiveTab('login'); }}>Login</a></p>
